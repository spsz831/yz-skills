...
python scripts/dedup_report.py <xlsx> --delete  # 确认后删除真重复行并重编号
```

删除前会打印清单并要求输入 `y`。默认不删，仅标记。`是` 含两种语义：`重复：`/同 URL 是真重复（可删），`与同表『X』同源` 是子页（保留）。
...
- `scripts/dedup_report.py` — 重复项报告 / 确认删除
- `scripts/cross_table_check.py` — 跨表一致性检查
- `references/rules.md` — 完整规范（表格结构 / 类型细分 / 名称引用 / 验证指标 / 排序 / 去重 / 跨表）
